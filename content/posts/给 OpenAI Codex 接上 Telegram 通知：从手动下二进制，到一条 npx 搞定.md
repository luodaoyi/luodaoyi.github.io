...
功能上它仍是通知。体验上，是把「必须盯着看」变成「可以先放手」。远程机器、长任务、串测试，手机能收到就行。

分发顺了之后，别人也更容易跟上：不用先挑平台、再改路径。

发布链路现在会构建 Windows、Linux、macOS 的 x64 和 ARM64，创建 GitHub Release，并发布主包和兼容包。核心是 Rust，包管理器只做分发。

## 最后

`go-codex-notify` 还是那个小问题：Codex 跑完先通知我。现在它是 Rust，版本 v1.3.23，渠道不止 Telegram，推荐命令是 `codex-notify`。
